...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = function (localeID, seed) {
		...
		if (seed) {
			if (_.isArray(seed) && seed.length > 0) mersenne.seed_array(seed);else mersenne.seed(seed);
		}

		self.random = {
			number: function number(min, max, precision) {
				min = min || 0;
				max = max || 99999;
		...
		self.get = function (format) {
			...
			if (format.indexOf("#{") != -1) {
				res = format.replace(/\#\{([A-Za-z_\.]+)\}/g, function (match, cap) {
					...
					if (part) {
						if (_.isArray(part)) return self.get(self.random.arrayElement(part));else _.isString(part);
